...
Decision-analytic models rely heavily on estimates from regression models. For example, both time to treatment discontinuation or patient survival are estimated using survival models, which can extrapolate outcomes beyond the available follow-up data. Below, we illustrate by estimating a Weibull model with the [flexsurv](https://github.com/chjackson/flexsurv-dev) package.

```{r survival, message=FALSE}
library("flexsurv")
...
### Beta distribution
The beta distribution is appropriate for describing the distribution of a probability or proportion. For instance, the beta distribution could be used to model uncertainty of the probabilities in a decision tree.
...
We can randomly sample transition probabilities using Dirichlet distributions with the function `rdirichlet_mat` from the [hesim](https://innovationvalueinitiative.github.io/hesim/) package (full discolusre, this package is developed by me). The function draws probabilities in each row in the transition matrix (i.e., transition probabilities from each state) from a separate Dirichlet distribution. 
...
```{r multion_plot, fig.width=6, fig.height=3}
tp.df <- data.frame(p = c(tp.sample), 
                    cat = rep(c(rep("1", 3), rep("2", 3), rep("3", 3)), dim(tp.sample)[3]),
                    state = rep(rep(paste0("Initial state = ", seq(1, 3)), 3), dim(tp.sample)[3])
)
