...
```{r diagram, out.width = "600px", echo = FALSE}
library("knitr")
knitr::include_graphics("figs/bone-marrow-tx-6state.png")
```
...
```{r tmat, warning=FALSE, message=FALSE}
library("mstate")
...
```{r estimation-data, warning=FALSE, message=FALSE}
library("mstate")
data("ebmt4")
...
For example, patient 2 began in state 1 (Tx) at time `0`. From state 1, there are 4 possible transitions: transition 2 (Tx to AE) was observed at time ```r subset(msebmt, id == 2 & status == 1)[1, "Tstop"]``` while transition 1 (Tx to Rec), transition 3 (Tx to Rel), and transition 4 (Tx to Death) were censored. The patient remained in state 3 until time ```r subset(msebmt, id == 2 & status == 1)[2, "Tstop"]```, when a transition to state 4 (AE and Rec) occurred and the transitions to states 5 (Rel) and 6 (Death) were censored. Patient 2 subsequently remained in state 4 until entering state 5 (Rel) at time ```r subset(msebmt, id == 2 & status == 1)[3, "Tstop"]```, at which time transition 12 (AE and Rec to Death) was censored.
...
```{r fitting, warning=FALSE, message=FALSE}
library("flexsurv")
...
```{r input-data, warning=FALSE, message=FALSE}
library("hesim")
...
```{r sim-comparison, cache=TRUE, cache.lazy=FALSE, include=FALSE}
...
```{r sim-comparison-plot, warning=FALSE, message=FALSE, fig.width=10, fig.height=7}
library("ggplot2")
...
```{r sim-timing, cache=TRUE, cache.lazy=FALSE}
...
The elapsed times (in seconds) suggest that `hesim` is ```r round(time_mstate[["elapsed"]]/time_hesim[["elapsed"]], 2)``` times faster than `mstate` when simulating a parametric multi-state model.
