...
### The Model
Let $d_{it}\equiv I(d_{it}^{\star} > 0) = I(y_{it} > 0)$ so that the latent variable $d_{it}^{\star}$ describes whether expenditures are positive or zero. The model can then be written as,

$$
\begin{aligned}
d_{it}^{\star} &= x_{1it}^{T}\alpha + b_{1i} + \epsilon_{1it},\\
\log (y_{it}|d_{it}^{*} >0) &=  x_{2it}^{T}\beta + b_{2i} + \epsilon_{2it},
\end{aligned}
$$
